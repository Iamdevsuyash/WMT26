...
original vocab.  Tokens added later via add_tokens() are NOT inserted into
that dict, so any newly added lang code (e.g. nif_Latn) returns None,
...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # lookup.  Tokens added via add_tokens() are absent from that dict,
        # so nif_Latn → None → int(None) → TypeError.
        #
